...
out vec4 position;			// vertex position in World Space
out vec4 normal;			// vertex normal in World Space
...
void main()
{
	...
	position = objectToWorld * vec4(vertexPositionObject, 1.0);
	normal = inverse(transpose(objectToWorld)) * vec4(vertexNormalObject, 0.0f);
